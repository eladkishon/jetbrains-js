...
export class WebstormRunConfiguration {
  private static buildEnvObject(env: {
  ...
  private static async saveRunConfig(fileName: string, content: unknown) {
    const runConfigXml = json2xml(JSON.stringify(content), {
      compact: true,
      spaces: 4,
    });
    ...
    fs.writeFileSync(
      `${ideaFolder}/runConfigurations/${fileName}.xml`,
      runConfigXml
    );
  }
  ...
  static async saveNodeJsConfig(options: RunConfigurationOptions) {
    ...
    Object.assign(
      runConfig.component.configuration.envs,
      WebstormRunConfiguration.buildEnvObject(options.env)
    );

    await WebstormRunConfiguration.saveRunConfig(
      `${options.name}-nodejs`,
      runConfig
    );
  }
  ...
  static async saveNpmConfig(options: NpmRunConfigurationOptions) {
    ...
    runConfig.component.configuration['package-json']._attributes.value =
      options.packageJsonPath;
    runConfig.component.configuration._attributes.name = packageJson.name;
    runConfig.component.configuration.envs.push(
      ...WebstormRunConfiguration.buildEnvObject(options.env)
    );
    await WebstormRunConfiguration.saveRunConfig(
      `${packageJson.name}-npmjs`,
      runConfig
    );
  }
}
